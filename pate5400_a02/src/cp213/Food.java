...
public class Food implements Comparable<Food> {
	...
	public int compareTo(final Food target) {
		...
		// your code here
		int name_Compare = this.name.compareTo(target.name);
		int Result = name_Compare;
		if (Result == 0) {
			int origin_Compare = Integer.compare(this.origin, target.origin);
			Result = origin_Compare;
		}
		return Result;
	}
	...
	public String getOriginString() {
		...
		// your code here
		String str = ORIGINS[this.origin];
	...
	public void write(final PrintStream ps) {
		...
		final String str = String.format("%s|%s|%b|%d", this.name, ORIGINS[this.origin], this.isVegetarian,
				this.calories);
		ps.append(str);
		return;
	}
